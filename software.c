...
int    prog_exit_progmode();
int    prog_enter_progmode();
...
int parse_hex (char * filename, unsigned char * progmem, unsigned char * config)
    ...
    while ((read =  getlinex(&line, &len, sf)) != -1)
        ...
        if (line_type==0)
            ...
            if(devid_expected == 0x1060 && effective_address>=flash_size){
                if (verbose>2) printf("CB for PIC16F628A");
                printf(" line address: %d ", (line_address&0x3FFF));
                for (i=0; i<line_len; i++) config[(line_address&0x3FFF)+i-0x0E] = line_content[i];
            }
            }
        if (line_type==4)
            {
...
//    for (i=0; i<10; i++) printf ("%2.2x",config_bytes[i]);
    ...
    printf("flash size: %d\n\n",flash_size);
        ...
        for(int i=0; i<2048; i++){
          if(i%16 == 0) printf("\noffset %X: ", i*2);
          printf("%04X ", progmem[i*2] | ((unsigned int)progmem[i*2+1]<<8));
        }
        printf("\n\ncfg : %04X\n",config_bytes[0] | ((unsigned int)config_bytes[1]<<8));

    if(program){
        printf("\n\n");
        for(int i=0; i<2048; i++){
            putByte(progmem[i*2]);putByte(progmem[i*2+1]);
            getByte();
        }
    ...
    for(int i=0; i<2048; i++){
        unsigned int d = 0;
      d = getByte();
      d |= ((unsigned int)getByte())<<8;
      if(verify){
        if(d != (0x3FFF&(progmem[i*2] | (((unsigned int)progmem[i*2+1]))<<8))  ){
            printf("Verify error! %04X != %04X\n", d, (0x3FFF&(progmem[i*2] | (((unsigned int)progmem[i*2+1]))<<8)));
            verify_error = 1;
        }
      }
      printf("%04X ", d);
    ...
    printf("\nCFG:\n");
    if(program){
        putByte(config_bytes[0]);putByte(config_bytes[1]);
    }
    for(int i=0; i<8; i++){
        unsigned int d = 0;
      d = getByte();
      d |= ((unsigned int)getByte())<<8;
      if(verify && i==7){
        if(d != (0x3FFF&(config_bytes[0] | (((unsigned int)config_bytes[1]))<<8))   ){
            printf("CFG verify error! %04X != %04X\n", d, (0x3FFF&(config_bytes[0] | (((unsigned int)config_bytes[1]))<<8)));
            verify_error = 1;
        }
      }
      printf("%04X ", d);
